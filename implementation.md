...
- [x] Dock/taskbar icon fixed: `QGuiApplication::setDesktopFileName("mdit")` (Wayland app id <-> `mdit.desktop`), `StartupWMClass=mdit`, user-level install (`cmake --install build --prefix ~/.local` -> `~/.local/bin/mdit` + hicolor icons + desktop entry). Verified GTK resolves `mdit` to our PNG and `mdit.desktop` is registered as the `text/markdown` handler (`gio mime text/markdown`). Note: `xdg-mime query default text/markdown` also prints `mdit.desktop`, but its generic fallback just returns the first `mimeinfo.cache` entry and is NOT proof of being the default — the install sets no default; the desktop decides (see AGENTS.md).
